...
public class featureSelectionProcessClass {
	...
	private  Instances dataset,resultInfoGain,resultGainRatio,resultSymmetri,resultreliefF,resultOneR;
	...
	public featureSelectionProcessClass(Instances dataset,int seed,int attr_num) throws Exception{
	...
	private Instances ensembleFeatureSelection() throws Exception{
		...
		dataProcess[3] = resultreliefF;
		dataProcess[4] = resultOneR;
		...
		for(int i=0 ; i < numDataProcess; i++ ){
			...
			for(int j=0 ; j < numAttr ;j++){//Start Inner Loop
				...
				setAverageRankingOfAttribute(dataProcess[i],j,j+1);
		...
		//Sort E List
		Collections.sort(E_List, eListClass.SortAttribute);
	...
	//Set average ranking order
	private void setAverageRankingOfAttribute(Instances dataProcess,int att_no,int rank){
		
		if(!average_ranking.containsKey( dataProcess.attribute(att_no) )){
			if(att_no < select_num){
			average_ranking.put(dataProcess.attribute(att_no) ,1.0);
			}
			...
			average_ranking.get(dataProcess.attribute(att_no))+1.0);
			}
		}
